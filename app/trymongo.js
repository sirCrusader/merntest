...
function usage() {
    console.log('Usage: ');
    console.log('node', __filename__, '<option>');
...
function testWithCallbacks() {
    MongoClient.connect('mongodb://172.17.0.2/playground', function (err, db) {
...
function testWithPromises() {
    let db;
    MongoClient.connect('mongodb://172.17.0.2/playground').then(connection => {
        db = connection;
...
function testWithGenerator() {
    const co = require('co');
    co(function* () {
        const db = yield MongoClient.connect('mongodb://172.17.0.2/playground');
...
function testWithAsync() {
    ...
    async.waterfall([
        next => {
            MongoClient.connect('mongodb://172.17.0.2/playground', next);
        },
        (connection, next) => {
            db = connection;
